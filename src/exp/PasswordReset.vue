<template>
  <div id="password-reset-view">
    ...
    <template v-else-if="!emailCompleted">
      ...
      <button @click="sendResetEmail(inputs)">send email</button>
      <span class="error" v-show="emailError">A error occured while processing your request.</span>
    </template>
    <template v-else>
      <div>
...
export default {
  ...
  methods: mapActions('password', ['sendResetEmail', 'clearEmailStatus']),
